...
% metodologia para cálculo do tempo rodar 4 vezes, tomar a média das 3 últimas
...
O resultado da consulta \#1, executada na base de dados de \(D^{\dagger}\) carregada no MonetDB, está disponível no \autoref{lst:experiments-1-sqlresults}. Uma representação gráfica dos conjuntos de dados de \(S^{\dagger}\) utilizados nessa consulta estão realçados em vermelho na \autoref{fig:experiments-dataflow-1}.
...
O objetivo da terceira consulta é TODO. A especificação completa da consulta está disponível na \autoref{tab:experiments-3-especificacao}. A consulta \#3 em SQL gerada pelo Query Processor está listada no \autoref{lst:experiments-3-sql}.
...
\texttt{dsDestinations}     & \{\texttt{}\} \\ \hline
\texttt{type}               & logical \\ \hline
\texttt{projections}        & \{\texttt{}\} \\ \hline
\texttt{selections}         & \varnothing \\ \hline
\texttt{dsIncludes}         & \varnothing \\ \hline
\texttt{dsExcludes}         & \varnothing \\
    \end{tabular}
...
\begin{minipage}[c]{0.95\textwidth}
\begin{lstlisting}[language=sql,label={lst:experiments-3-sql},caption={[Código em SQL gerado na consulta~\#3]Código em SQL gerado na consulta~\#3 (tempo médio: TODO~ms).}]
SELECT TODO
\end{lstlisting}
\end{minipage}

O resultado da consulta \#3, executada na base de dados de \(D^{\dagger}\) carregada no MonetDB, pode ser visualizado no \autoref{lst:experiments-3-sqlresults}: suas colunas estão na mesma ordem da especificação das projeções em \autoref{lst:experiments-3-sql}. Uma representação gráfica dos conjuntos de dados de \(S^{\dagger}\) utilizados nessa consulta estão realçados em vermelho na \autoref{fig:experiments-dataflow-3}.

\begin{lstlisting}[language=sqlresults,label={lst:experiments-3-sqlresults},caption={[Resultados da consulta \#3.]Resultados da consulta \#3 (TODO tuplas, tempo médio: TODO~ms).}]
TODO
\end{lstlisting}
