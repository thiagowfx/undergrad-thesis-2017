...
O \textbf{Provenance Data Gatherer} (PDG) é o componente responsável por capturar proveniência (\textit{c.f.} \autoref{subsec:ingestao-de-dados}) e dados específicos do domínio a partir do código da aplicação, gerando um arquivo JSON com o conteúdo extraído e as suas respectivas dependências~\cite{silva2016situ}. Após a captura, a proveniência e os dados de domínio são carregados em um banco de dados que é compatível com um esquema (do inglês \textit{schema}) pré-definido e que é gerenciado pelo SGBD MonetDB~\cite{boncz2008breaking}, que é um banco de dados orientado a colunas compatível com a linguagem SQL.
...
O \textbf{Query Processor} (QP) é o componente responsável por auxiliar os usuários a submeter consultas na linguagem SQL ao banco de dados de proveniência~\cite{silva2016situ}, realizar a validação dessas consultas --- por exemplo, se a sintaxe e~/~ou se os nomes dos atributos estão corretos --- e então finalmente retornar e exibir os resultados das mesmas.
